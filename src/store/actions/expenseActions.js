        ...
        formData.append('image', {
          ...
          type:
            receiptFile.type === 'pdf'
              ? 'application/pdf'
              : `image/${receiptFile.type || 'jpeg'}`,
          name:
            receiptFile.name ||
            `receipt_${Date.now()}.${receiptFile.type || 'jpg'}`,
        });
...
export const fetchExpenses = () => async dispatch => {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      // API response structure: { message, status, data: [...] }
...
export const fetchExpenseById = expenseId => async dispatch => {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      let expenseItem =
        response.data?.data || response.data?.expense || response.data;
      ...
      throw new Error(
        response.data?.message || 'Failed to fetch expense details',
      );
    }
  } catch (error) {
